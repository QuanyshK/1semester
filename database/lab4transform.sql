...
alter table Books drop available_copies;
select count(*) as bookcount from Books where publication_year<2000;
select title, publication_year, case when publication_year < 1920 then 'category 1' when publication_year  between 1920 and 1980 then 'category 2' when publication_year > 1980 then 'category 3' end as category from Books;
select upper(first_name) as uppercase from customers;
select count(distinct customer_ID) as bookborrow from Customers;
select * from Authors where first_name = '_%.%';
select * from Customers order by last_name limit 10;
select substring(first_name, 0, 4) from Authors;
...
select title || '-' || publication_year as titlepublic from Books;
select customer_ID, first_name, last_name, substring(phone_number, 6) from Customers;
/*dfgf*/
select borrow_ID, book_ID, customer_ID, borrow_date, return_date, (return_date - borrow_date)  interval from Borrowed_books;
select first_name, last_name, char_length((format('%s %s', first_name, last_name))) from Authors;
select publication_year, count(*)  from Books group by publication_year having count(*) > 3;
/*lab5*/
select distinct b.title from Books AS b left join Borrowed_books as bb on b.book_ID = bb.book_ID where bb.book_ID is null;
select b.title from Books AS b join Authors as a on b.author_ID = a.author_ID where extract(year from a.birth_date) < 1930;
 insert into Authors(author_ID, first_name, last_name, birth_date)
     values
 (21, 'Haruki', 'Murakami', '1949-01-12'),
 (22, 'Nassim', 'Taleb', '1960-09-11'),
 (23, 'Ray', 'Bradbury', '1920-08-22');
select a.author_ID, a.first_name, a.last_name from Authors as a left join Books as b on a.author_ID = b.author_ID where b.book_ID is null;
select a.author_ID, a.first_name, a.last_name, count(b.book_ID) as published_books from Authors as a left join Books as b on a.author_ID = b.author_ID
group by a.author_ID, a.first_name, a.last_name
order by published_books desc
limit 15;
select c.first_name, c.last_name, b.title from Customers AS c join Borrowed_books as bb on c.customer_ID = bb.customer_ID join Books as b on bb.book_ID = b.book_ID where b.title like '%Potter%';
...
select distinct bb.book_ID
from Borrowed_books as bb
join Customers as c on bb.customer_ID = c.customer_ID
where c.last_name = 'Ospanov';
SELECT DISTINCT bb.book_ID
FROM Borrowed_books AS bb
WHERE EXTRACT(DOW FROM bb.borrow_date) = 1;